...
export default function TripDetail({ params }: { params: { id: string } }) {
  ...
  async function aceptar(uid: string) {
    ...
    await supabase.from('trips')
      .update({ plazas_disponibles: trip.plazas_disponibles - 1 })
      .eq('id', params.id);
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 space-y-4">
      ...
      {!soyCreador && !yaSoyAlgo && (
        <button onClick={solicitar} className="bg-teal-600 text-white px-4 py-2 rounded">
          Solicitar unirse
        </button>
      )}
          ...
          {requests.map(r => (
            <div key={r.user_id} className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={()=>aceptar(r.user_id)} className="bg-teal-600 text-white px-3 py-1 rounded">Aceptar</button>
      ...
      <a className="text-teal-600 underline" href={`/chat/${trip.id}`}>Ir al chat del viaje</a>
    </div>
  );
}
