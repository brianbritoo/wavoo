...
export default function TripDetail({ params }: { params: { id: string } }) {
  ...
  async function solicitar() {
    ...
    const { error } = await supabase
      .from('trip_members')
      .insert({ trip_id: params.id, user_id: userId, estado: 'pendiente', rol: 'miembro' });
  ...
  async function aceptar(uid: string) {
    ...
    await supabase
      .from('trips')
      .update({ plazas_disponibles: (trip.plazas_disponibles ?? 0) - 1 })
      .eq('id', params.id);
    setStatus('✅ Solicitud aceptada');
    await loadTrip(); await loadRequests(); await loadMembers();
  }
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 space-y-4">
      {/* Encabezado (antes estaba fuera del componente) */}
      ...
      <p>Plazas disponibles: {trip.plazas_disponibles ?? 0}</p>

      {/* Acciones */}
      {!soyCreador && !yaSoyAlgo && (
        <button onClick={solicitar} className="bg-brand text-white px-4 py-2 rounded">
          Solicitar unirse
        </button>
      )}
          ...
          {requests.map(r => (
            <div key={r.user_id} className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={() => aceptar(r.user_id)} className="bg-brand text-white px-3 py-1 rounded">
                  Aceptar
                </button>
                <button onClick={() => rechazar(r.user_id)} className="bg-gray-300 px-3 py-1 rounded">
                  Rechazar
                </button>
              </div>
            </div>
